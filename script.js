...
let activeLkpdScale = 1.0;
let activeLkpdFitMode = 'fit';
let currentActiveLkpdContext = { ptmId: null, idLkpd: null, pdfDoc: null, fieldMap: null, savedAnswers: {} };
...
async function renderLkpdUntukSiswa(containerEl, lkpdObj, ptmId) {
  ...
  try {
    ...
    currentActiveLkpdContext = { ptmId, idLkpd: lkpdObj.id_lkpd, pdfDoc, fieldMap, savedAnswers: combinedAnswers };
...
async function renderDesktopInlinePdfPages(containerWrap, pdfDoc, fieldMap, savedAnswers, idLkpd) {
...
async function openLkpdFullscreenModal(ptmId, idLkpd) {
  const lkpdObj = (state.cachedData.lkpd || []).find((l) => String(l.id_lkpd) === String(idLkpd));
  if (!lkpdObj) return;
  ...
  if (currentActiveLkpdContext.pdfDoc && currentActiveLkpdContext.idLkpd === idLkpd) {
    ...
    currentActiveLkpdContext = { ptmId, idLkpd, pdfDoc, fieldMap, savedAnswers: combinedAnswers };
    await drawLkpdFullscreenPages(contentWrap);
  }
}

function closeLkpdFullscreenModal() {
  const modal = document.getElementById('lkpd-fullscreen-modal');
  modal.classList.add('hidden');
  modal.classList.remove('flex');
}

async function drawLkpdFullscreenPages(containerEl) {
  const { pdfDoc, fieldMap, savedAnswers, idLkpd } = currentActiveLkpdContext;
  ...
  if (activeLkpdFitMode === 'fit') {
    ...
    activeLkpdScale = renderScale;
  } else {
    renderScale = activeLkpdScale;
  }
...
function setLkpdZoomMode(mode) {
  activeLkpdFitMode = mode;
  const contentWrap = document.getElementById('lkpd-fullscreen-content');
  if (contentWrap) drawLkpdFullscreenPages(contentWrap);
}

function changeLkpdZoom(delta) {
  activeLkpdFitMode = 'custom';
  activeLkpdScale = Math.min(Math.max(activeLkpdScale + delta, 0.4), 2.5);
  const contentWrap = document.getElementById('lkpd-fullscreen-content');
  if (contentWrap) drawLkpdFullscreenPages(contentWrap);
}
...
async function submitReviewMateri(idMateri, ptmId) {
  if (!state.currentUser) {
    requireStudentAuth();
    return;
  }
